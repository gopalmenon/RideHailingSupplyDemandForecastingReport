...
\begin{multicols}{2}
\vspace{1em}
Since we were not getting good results, we decided to look for outliers. Outliers are usually computed using the inter-quartile distance between the first and third quartiles and using that value to include elements that are up to $1.5$ times the inter-quartile distance on either side. Elements outside these limits are marked as outliers. However it was doubtful that the reported outliers were accurate since it is actual data and the reported number was large.  \\

We next looked at clustering methods to determine and filter out outliers. After experimenting with a couple of methods, there was no improvement in the error. We confirmed our theory that the identified outliers are most likely not true outliers, but result of the scattered nature of the dataset. \\
